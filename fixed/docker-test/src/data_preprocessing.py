...
def ensure_channels_last(x: np.ndarray) -> np.ndarray:
    print(2333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333)
    print(x.shape)
...
# def ensure_channels_last_tensor(x: tf.Tensor) -> tf.Tensor:
#     """Move the channel dimension of a 4D tensor to the last axis if needed.

#     This mirrors :func:`ensure_channels_last` for TensorFlow tensors so that
#     symbolic inputs used to build a model are guaranteed to be NHWC before they
#     are passed to ViT. If the channel axis is already last, the tensor is
#     returned unchanged.
#     """
#     print(x.shape)
...
#     if (
#         static_shape.rank == 4
#         and static_shape[3] == 3
#     ):
#         # Already (None, H, W, 3) ― skip any further processing.
#         return x
...
#         # Fallback for dynamic shapes where static inspection failed.
#         dynamic_shape = tf.shape(x)
#         matches = [tf.equal(dynamic_shape[i], 3) for i in range(1, 4)]

#         def _transpose(idx: int) -> tf.Tensor:
#             perm = [0] + [ax for ax in (1, 2, 3) if ax != idx] + [idx]
#             return tf.transpose(x, perm)

#         return tf.case(
#             [(matches[i], lambda i=i: _transpose(i + 1)) for i in range(3)],
#             default=lambda: x,
#             exclusive=True,
#         )
...
#     perm = [0] + [ax for ax in (1, 2, 3) if ax != channel_axis] + [channel_axis]
#     return tf.transpose(x, perm)
def ensure_channels_last_tensor(x):
    # 4次元テンソルだけ確認して、形だけ保証して何もしない
    x = tf.ensure_shape(x, (None, common.IMG_HEIGHT, common.IMG_WIDTH, 3))
    return x
