...
from data_preprocessing import ensure_channels_last, ensure_channels_last_tensor
...
def create_model(input_tensor, model_params):
    ...
    # ==========================
    if model_name == 'ViT':
        # 入力は従来どおり channels_last で定義
        inputs = tf.keras.Input(
            shape=(common.IMG_HEIGHT, common.IMG_WIDTH, 3),
            name="input_image"
        )

        # ViT が要求する (B, C, H, W) = (B,3,224,224) に変換
        vit_inputs = tf.keras.layers.Lambda(
            lambda x: tf.transpose(x, (0, 3, 1, 2)),
            name="to_channels_first"
        )(inputs)

        # Hugging Face の ViT モデルをロード
        base_model = TFAutoModelForImageClassification.from_pretrained(
            common.VIT_MODEL_NAME,
            from_pt=True,
        )

        # ViT 本体を呼び出し (pixel_values で渡すのがポイント)
        outputs = base_model(pixel_values=vit_inputs, training=True)
        x = outputs.logits  # (batch_size, hidden_size)

        # 必要に応じて Dense / Dropout を追加
        if model_params['num_dense_layers'] > 0:
            for _ in range(model_params['num_dense_layers']):
                x = tf.keras.layers.Dense(dense_units, activation=activation_func)(x)
                x = tf.keras.layers.Dropout(dropout_rate)(x)

        # 最終出力
        x = tf.keras.layers.Dense(common.NUM_CLASS, activation='softmax')(x)
