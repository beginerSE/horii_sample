...
#class MyCallback(keras.callbacks.Callback):
    def __init__(self, model, patience, stop_patience, threshold, factor, batches, epochs, ask_epoch):
        ...
        #self.model = model
        self._model = model  # プライベート変数として model を保持
    ...
    @property
    def model(self):
        return self._model  # getter: _model を返す
    @model.setter
    def model(self, value):
        self._model = value  # setter: _model に値を設定する
        ...
        if acc < self.threshold: # if training accuracy is below threshold adjust lr based on training accuracy
            ...
            if acc > self.highest_tracc: # training accuracy improved in the epoch
                ...
                if self.count >= self.patience - 1: # lr should be adjusted
                    ...
                    #    lr = float(lr)
                    #except ValueError:
                    #    raise ValueError(f"Learning rate '{lr}' could not be converted to a float.")
                    #
                    #if isinstance(self.model.optimizer.learning_rate, tf.keras.optimizers.schedules.LearningRateSchedule):
                    #    print("isinstance \n\n")
                    #    self.model.optimizer.learning_rate = tf.keras.optimizers.schedules.ExponentialDecay(initial_learning_rate=lr, 
                    #                                                                                        decay_steps=100000, 
                    #                                                                                        decay_rate=0.96, 
                    #                                                                                        staircase=True)
                    #else:
