...
def create_model(input_tensor, model_params):
    ...
    # ベースモデルを選択
    #if model_name in common.available_models:
    #    base_model_class = common.available_models[model_name]
    ...
    if model_name == 'ViT':
        ...
        outputs = base_model(pixel_values=inputs, training=False)
    ...
    # パラメタライズされた層設定
    dense_units = model_params['dense_units']  # Dense層のユニット数
    dropout_rate = model_params['dropout_rate']  # Dropout率
    activation_func = model_params['activation_func']  # 活性化関数 
    optimizer = get_optimizer(model_params['optimizer_func'], model_params['learning_rate'])# 最適化活関数
    fix_layer = model_params['fix_layer']  # 重み固定のレイヤー指定

    # 層を追加 (複数のDense層の追加にも対応)
    if model_params['num_dense_layers'] > 0:
        for _ in range(model_params['num_dense_layers']):
            top_model.add(Dense(dense_units, activation=activation_func))  # Dense層
            top_model.add(Dropout(dropout_rate))  # Dropout層

    top_model.add(Dense(common.NUM_CLASS, activation='softmax'))  # 最後の出力層
    ...
    if common.ENABLE_FIX_LAYER:
        # 重みレイヤ指定
        #for layer in base_model.layers[:common.FIX_LAYER]:
        fix_layer_set = -1 * fix_layer # 最後のx層を学習させる
        if fix_layer_set==0:
            for layer in base_model.layers:
                layer.trainable = False
        else:
            for layer in base_model.layers[:fix_layer_set]: # 最後のx層を学習させる
                layer.trainable = False
    else:
        # ベースモデル全固定
        for layer in base_model.layers:
